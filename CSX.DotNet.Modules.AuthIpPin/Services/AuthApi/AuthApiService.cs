...
public partial class AuthApiService : IAuthApiService
{
    // Infrastructure

    private readonly IAuthDbWorkerService _db;
        ...
        await _db.ExecuteAsync(async (db, ct) =>
        {
                ...
                session.PinExpiry.Value > now)
            {
                ...
                if (dto.RequestPayload == session.Pin.Value)
                {
                    ...
                    string authToken = AuthApiExtensions.GenerateAuthToken();
